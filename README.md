...
```mermaid
flowchart LR
  T["Talent"] --> F["Faculty<br/>real market demand"]
  F --> P["Portfolio<br/>working systems"]
  P --> V["Verification<br/>coach + certification"]
  V --> M["Matching"]
  M --> C["Company"]
  C -- "feedback on the talent" --> V
  C -- "what the role actually needed" --> F
  M -- "feedback on the company" --> T
```
